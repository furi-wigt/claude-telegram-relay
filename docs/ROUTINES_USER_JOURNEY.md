...
4. [Creating Code-Based Routines via /plan](#3b-creating-code-based-routines-via-plan)
5. [Managing Routines via /routines](#4-managing-routines)
...
```javascript
{
  ...
  interpreter: "/Users/<username>/.bun/bin/bun",
  exec_mode: "fork",
  cwd: "/path/to/project",
  instances: 1,
  autorestart: false,
...
## 3b. Creating Code-Based Routines via /plan

This is the fastest way to create a new code-based routine without leaving Telegram. Use `/plan` to describe what you want — Claude generates the TypeScript file through an interactive Q&A, you save it to `routines/`, then register it via `/routines list`.

### Real example (from PM2 logs, 2026-02-19 16:19)

```
You:  /plan build hello world in nodejs as daily routine

Bot:  📋 New task started!
      What type of output should it send?
      [1] Just log to console  [2] Send a Telegram message

You:  2

Bot:  What time should it run daily? (e.g. "9am", "every 30 minutes")

You:  9am

Bot:  Any specific message to send, or just "Hello World"?

You:  Just "Hello World" with today's date

Bot:  ✅ Executing…
```

Claude then generates the full TypeScript routine and streams it back. From the logs, the session ran for ~4 minutes (`16:19:08` → `16:23:18`) and produced a working file.
...
```
/plan build a daily hello world routine in nodejs that sends a Telegram message every morning
```

`/plan` opens the interactive Q&A. Claude asks 2–4 clarifying questions (schedule, output target, message content). Answer each with the inline buttons or free text.

**Step 2 — Claude generates the code**

After the final confirmation, Claude implements the routine. The response includes the complete TypeScript file:

```typescript
#!/usr/bin/env bun

/**
 * @routine hello-world-daily
 * @description Sends "Hello World" with today's date every morning
 * @schedule 0 9 * * *
...
const CHAT_ID = Number(process.env.TELEGRAM_USER_ID);

async function main() {
  const today = new Date().toLocaleDateString("en-SG", { timeZone: "Asia/Singapore" });
  await sendToGroup(CHAT_ID, `Hello World — ${today}`);
}
...
Copy the generated code into `routines/hello-world-daily.ts`.

**Step 4 — Test it manually**

```bash
bun run routines/hello-world-daily.ts
```

Check your Telegram — the message should arrive instantly.

From the real session logs:
```
16:23:18  [Code Quality & TDD Coach] Run it now to test
16:24:10  [Code Quality & TDD Coach] Hello-world.ts
```

The user sent the file to the Code Quality group for a quick review before registering it.

**Step 5 — Register via /routines list**

```
/routines list
```

The bot detects the new unregistered file and offers an inline button:

```
⚠️ 1 unregistered routine found:
  • hello-world-daily

[Register hello-world-daily]  [Skip]
```

Tap **Register hello-world-daily**. The bot asks for a cron expression:

```
Bot:  What schedule for "hello-world-daily"?
      Send a cron expression (e.g. `0 9 * * *` for 9am daily).

You:  0 9 * * *

Bot:  ✅ Routine "hello-world-daily" registered with schedule: `0 9 * * *`
      Run /routines list to see its status.
```

Alternatively, skip the list and register directly:

```
/routines register hello-world-daily 0 9 * * *
```
...
| Tip | Detail |
|-----|--------|
| Be specific about schedule | Say "9am SGT daily" not just "morning" — Claude uses SGT by default |
| Include the word "routine" | Helps Claude understand this is a scheduled script, not a one-off |
| Test before registering | `bun run routines/<name>.ts` catches errors instantly |
| Add JSDoc tags | `@routine`, `@description`, `@schedule`, `@target` — these power `/routines list` display |
| Use existing utilities | `sendToGroup`, `sendAndRecord`, `validateGroup` are already in `src/utils/` |
