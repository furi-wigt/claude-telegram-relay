  ...
  const parseStream = async (): Promise<void> => {
    ...
    try {
      while (true) {
        ...
        buf = lines.pop() ?? "";
        // M-LEAK: Cap incomplete-line buffer to prevent unbounded growth from malformed output
        if (buf.length > 1_048_576) {
          console.warn(`[stream] parseStream buf exceeded 1MB (${buf.length} bytes) — truncating`);
          buf = buf.slice(-1_048_576);
        }
        ...
        for (const line of lines) {
          ...
          if (type === "system" && event.subtype === "init" && typeof event.session_id === "string") {
            ...
            if (text) {
              ...
              // M-LEAK: Cap assistant text accumulator — only used as fallback for resultText
              if (lastAssistantText.length > 102_400) {
                lastAssistantText = lastAssistantText.slice(-102_400);
              }
            ...
            // M-LEAK: Free duplicate buffer — resultText now holds the final answer
            lastAssistantText = "";
      ...
      reader.cancel(); // M-3: release reader lock on all exit paths
    }
  };
