...
// ── Mock storageBackend and db so tests don't hit SQLite/Qdrant/Ollama ─────

const mockInsertMemoryRecord = mock(async () => ({ id: "test-id", error: null }));
const mockDeleteMemoryRecord = mock(async () => {});
const mockUpdateMemoryRecord = mock(async () => {});
const mockInsertMessageRecord = mock(async () => {});

// Mock DB for query-based operations in directMemoryCommands
const mockDbQuery = mock((_sql: string) => ({
  get: mock((..._args: any[]) => null),
  all: mock((..._args: any[]) => []),
  run: mock((..._args: any[]) => {}),
}));
const mockDb = { query: mockDbQuery, run: mock(() => {}) };
...
// Global mock reset — ensures each test starts with clean call counts
beforeEach(() => {
  mockInsertMemoryRecord.mockClear();
  mockDeleteMemoryRecord.mockClear();
...
/**
 * Mock that configures the DB mock to return given candidates for queries.
 * The production code uses getDb().query(...).all() to fetch items.
 */
function mockWithCandidates(candidates: Array<{ id: string; content: string }>) {
  const eqDeleteFn = mock(() => {});

  mockDbQuery.mockImplementation((_sql: string) => ({
    get: mock((..._args: any[]) => candidates[0] ?? null),
    all: mock((..._args: any[]) => candidates),
    run: mock((..._args: any[]) => {}),
  }));
  ...
  return {
    eqDeleteFn,
    chain: { limit: mock(() => candidates) },
    listChain: { limit: mock(() => candidates) },
  };
}

/** Reset DB mock for simple add-only tests */
function resetMocksSimple() {
  mockInsertMemoryRecord.mockClear();
  mockInsertMessageRecord.mockClear();
  mockDbQuery.mockImplementation((_sql: string) => ({
    get: mock((..._args: any[]) => null),
...
describe("/goals command — add path", () => {
  test("lists goals (not usage) when no argument given", async () => {
    const bot = mockBot();
    // listItems needs select; use mockForList from the no-args section
    const listChain: any = {};
  ...
  test("inserts a goal with correct type and category", async () => {
    ...
    const inserted = mockInsertMemoryRecord.mock.calls[0][0];
  ...
  test("inserts multiple goals from one command", async () => {
    ...
    expect(mockInsertMemoryRecord).toHaveBeenCalledTimes(2);
    const first = mockInsertMemoryRecord.mock.calls[0][0];
    const second = mockInsertMemoryRecord.mock.calls[1][0];
...
describe("/goals command — remove path (ilike fallback)", () => {
  test("removes immediately when single candidate matches", async () => {
    ...
    // Should delete the matched item via deleteMemoryRecord
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("g1");
...
describe("/goals remove path — markdown normalization", () => {
  test("removes goal with backtick-wrapped terms when query has no backticks", async () => {
    const bot = mockBot();
    const { eqDeleteFn } = mockWithCandidates([
      {
        id: "g1",
  ...
  test("removes goal with asterisk markdown when query has no asterisks", async () => {
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("g2");
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Removed");
  });
  ...
  test("removes goal with mixed markdown (backticks + underscores) when query is plain text", async () => {
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("g3");
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Removed");
  });
  ...
  test("markDone (*) works for goal stored with backtick markdown", async () => {
    ...
    // Mock DB to return the goal for findMatchingItems and toggleGoalDone
    mockDbQuery.mockImplementation((sql: string) => ({
      get: mock((..._args: any[]) => {
        // toggleGoalDone queries for type
        if (sql.includes("type")) return { type: "goal" };
        return { id: "g4", content: "Implement `_isEntry` guard for PM2/bun" };
      }),
      all: mock((..._args: any[]) => [
        { id: "g4", content: "Implement `_isEntry` guard for PM2/bun" },
      ]),
      run: mock((..._args: any[]) => {}),
    }));
...
describe("dmem callback handlers", () => {
  test("dmem_del deletes item by ID and edits message", async () => {
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("item-uuid-456");
  ...
  test("dmem_del always works with local storage", async () => {
    const bot = mockBot();
    registerDirectMemoryCommands(bot as any, {});
    ...
    // Storage is always available (local SQLite), so deletion succeeds
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("some-id");
    expect(ctx.answerCallbackQuery).toHaveBeenCalled();
  });
});
...
describe("commands always work with local storage", () => {
  test("adds items successfully for all commands", async () => {
    const bot = mockBot();
    registerDirectMemoryCommands(bot as any, {});
    ...
    for (const cmd of ["goals", "facts", "prefs", "reminders"]) {
      ...
      // Storage is always local — commands should succeed
      expect(mockInsertMemoryRecord).toHaveBeenCalledTimes(1);
      ctx.reply.mockClear();
    }
  });
...
/** Build a DB mock for the list path.
 * Configures mockDbQuery to return the given items.
 */
function mockForList(items: Array<{ id: string; content: string }>) {
  const listChain: any = {};
  listChain.limit = mock(() => items);

  mockDbQuery.mockImplementation((_sql: string) => ({
    get: mock((..._args: any[]) => items[0] ?? null),
    all: mock((..._args: any[]) => items),
    run: mock((..._args: any[]) => {}),
  }));
  ...
  return {
    insertFn: mockInsertMemoryRecord,
    listChain,
  };
}
...
describe("/goals no-args — lists all goals", () => {
  ...
  test("saves list reply to STM via saveCommandInteraction", async () => {
    const bot = mockBot();
    mockForList([{ id: "g1", content: "My goal" }]);
    registerDirectMemoryCommands(bot as any, {});

    const ctx = mockCtx({ match: "" });
    await bot._triggerCommand("goals", ctx);

    // STM save: saveCommandInteraction calls insertMessageRecord twice (user + assistant)
    expect(mockInsertMessageRecord).toHaveBeenCalledTimes(2);
    const assistantCall = mockInsertMessageRecord.mock.calls[1][0] as { role: string; content: string };
    expect(assistantCall.role).toBe("assistant");
    expect(assistantCall.content).toContain("My goal");
  });
});
...
describe("E2E — findMatchingItems uses same scope as listItems", () => {
  test("findMatchingItems for /facts does NOT apply chat_id scope (provenance model: globally visible)", async () => {
    ...
    const bot = mockBot();
    const { chain } = mockWithCandidates([
  ...
  test("findMatchingItems for /facts finds items regardless of category", async () => {
    const bot = mockBot();
    mockWithCandidates([
      { id: "f1", content: "pm2 cron implementation" },
    ]);
    ...
    // SQLite query includes all categories (personal + null) — item is found and deleted
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("f1");
  });
  ...
  test("/facts -pm2 cron removes item even when stored with category=null (via [REMEMBER:] tag)", async () => {
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("f2");
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Removed");
...
describe("E2E — disambiguation keyboard shows correct items", () => {
  ...
  test("/facts -Claude relay deletes 'Claude relay status', not unrelated items", async () => {
    ...
    const { eqDeleteFn } = mockWithCandidates(candidates);
    registerDirectMemoryCommands(bot as any, {});
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("f4");
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Removed");
    expect(text).toContain("Claude relay status");
  });
...
describe("E2E — /goals shows items regardless of category (fix for [GOAL:] tag items)", () => {
   ...
   */
  test("listItems for goals shows all goals regardless of chatId (globally scoped)", async () => {
    const bot = mockBot();
    mockForList([
...
describe("E2E — /facts shows items without category (fix for [REMEMBER:] tag items)", () => {
  test("listItems for facts shows all facts including those without category", async () => {
    const bot = mockBot();
    mockForList([
      { id: "f1", content: "personal facts: name, age, location, job, family" },
    ...
    // All facts visible including those stored with category=null
    const text = ctx.reply.mock.calls[0][0] as string;
...
describe("index-based delete (-N syntax)", () => {
  ...
  test("/prefs -3 out of range (only 2 prefs) shows 'Not found'", async () => {
    ...
    expect(mockDeleteMemoryRecord).not.toHaveBeenCalled();
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Not found");
  });
  ...
  test("/reminders -2 deletes the 2nd reminder", async () => {
    ...
    expect(mockDeleteMemoryRecord).toHaveBeenCalledWith("r2");
    const text = ctx.reply.mock.calls[0][0] as string;
    expect(text).toContain("Removed");
...
describe("cache — multi-op index stability", () => {
  /**
   * Build a DB mock for cache-based tests.
   * Once firstToggledId is recorded in updatedIds (via mockUpdateMemoryRecord),
   * subsequent list queries exclude it — simulating the DB state where
   * that goal's type has been changed to completed_goal.
   */
  function mockMultiOp(
    ...
    mockUpdateMemoryRecord.mockImplementation(async (id: string) => {
      updatedIds.push(id);
    });

    mockDbQuery.mockImplementation((_sql: string) => ({
      get: mock((..._args: any[]) => {
        const item = goals.find(g => _args.includes(g.id));
        return item ? { ...item, type: "goal" } : null;
      }),
      all: mock((..._args: any[]) => {
        return updatedIds.includes(firstToggledId) ? goalsAfterToggle : goals;
      }),
      run: mock((..._args: any[]) => {}),
    }));
  ...
  test("/goals *1 | *3 — first toggle does not shift second toggle", async () => {
    ...
    const { updatedIds } = mockMultiOp(goals, "g1");
    const bot = mockBot();
    registerDirectMemoryCommands(bot as any, {});
  ...
  test("single-op /goals *2 still works after cache fix (regression)", async () => {
    ...
    const { updatedIds } = mockMultiOp(goals, "g2");
    const bot = mockBot();
    registerDirectMemoryCommands(bot as any, {});
  ...
  test("/facts -1 | -3 cold-start seeds cache on first op (no prior list call)", async () => {
    ...
    // Mock DB to return facts, simulating shrinking list as items are deleted
    const deletedIds: string[] = [];
    mockDbQuery.mockImplementation((_sql: string) => ({
      get: mock((..._args: any[]) => facts.filter((f) => !deletedIds.includes(f.id))[0] ?? null),
      all: mock((..._args: any[]) => facts.filter((f) => !deletedIds.includes(f.id))),
      run: mock((..._args: any[]) => {}),
    }));
    mockDeleteMemoryRecord.mockImplementation(async (id: string) => {
      deletedIds.push(id);
    });
...
describe("listItems — provenance model read path (R5)", () => {
  ...
  test("reminders list uses chat_id scope (dates stay chat-scoped)", async () => {
    // Reminders are chat-scoped — SQL query includes chat_id filter.
    const bot = mockBot();
    mockForList([]);
    registerDirectMemoryCommands(bot as any, {});
    ...
    // Verify the SQL query was called (behavioral: the command runs without error)
    expect(mockDbQuery).toHaveBeenCalled();
    // Check that the SQL contains chat_id filtering for reminders
    const sqlCalls = mockDbQuery.mock.calls.map((c: any) => c[0] as string);
    const hasChatIdFilter = sqlCalls.some((sql: string) => sql.includes("chat_id"));
    expect(hasChatIdFilter).toBe(true);
  });
});
