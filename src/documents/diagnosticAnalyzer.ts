 ...
 *   1. ~/.claude-relay/prompts/diagnostics/<agent-id>.md  (user override)
 *   2. config/prompts/diagnostics/<agent-id>.md           (repo default)
 *   3. BUILT_IN_DEFAULTS[agentId]                         (hardcoded default — works out of the box)
 *   4. Generic fallback                                   (unknown agentId)
 *
 * To customize for your deployment: create ~/.claude-relay/prompts/diagnostics/<agent-id>.md
 * and it will be used in place of the repo/built-in defaults. No TypeScript changes needed.
 */
 ...
 *
 * Priority order:
 * 1. ~/.claude-relay/prompts/diagnostics/<agentId>.md (user override)
 * 2. config/prompts/diagnostics/<agentId>.md          (repo default)
 * 3. BUILT_IN_DEFAULTS[agentId]                       (hardcoded)
 * 4. undefined (caller should apply generic fallback)
 */
export function loadExtractionPrompt(
  ...
  // 1. Try user-customised diagnostic prompt
  const userPath = join(getUserPromptsDir(), "diagnostics", fileName);
  ...
  // 2. Try repo default diagnostic prompt
  const root = projectRoot ?? PROJECT_ROOT_DEFAULT;
  const repoPath = join(root, "config", "prompts", "diagnostics", fileName);
  try {
