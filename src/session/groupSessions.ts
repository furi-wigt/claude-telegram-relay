...
/** Ensure sessions directory exists (sync, cached after first check). */
let _dirVerified = false;
function ensureSessionsDir(): void {
  if (_dirVerified) return;
...
export interface SessionState {
  ...
  /**
   * When true, this session was created by an explicit /new reset.
   * Used to scope short-term memory queries to only post-reset messages.
   * Set by resetSession(), never set on natural session creation.
   */
  wasExplicitReset?: boolean;
  /**
...
function evictIfNeeded(): void {
  if (sessions.size > MAX_SESSIONS_CACHE) {
    // M-LEAK: Evict 10% of oldest entries (FIFO from Map insertion order)
    const toEvict = Math.ceil(MAX_SESSIONS_CACHE * 0.1);
    const keys = sessions.keys();
    for (let i = 0; i < toEvict; i++) {
      const k = keys.next().value;
      if (k !== undefined) sessions.delete(k);
    }
  }
}
 ...
 */
export function getSessionSince(session: SessionState): string | undefined {
