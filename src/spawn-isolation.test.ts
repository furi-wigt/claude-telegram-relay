...
describe("ecosystem.config.cjs — PM2 configuration integrity", () => {
  ...
  test("max_memory_restart is 1500M (not the too-low 500M that caused restart loop)", () => {
    // Parse the config to check telegram-relay specifically (other services may use 500M)
    const config = require(ecosystemPath);
    const relay = config.apps.find((a: any) => a.name === "telegram-relay");
    expect(relay).toBeDefined();
    expect(relay.max_memory_restart).toBe("1500M");
  });
