...
describe("claudeStream — stream completes before signal fires", () => {
  ...
  test("abort after completion does not call kill on the completed proc", async () => {
    ...
    // kill is called exactly once by the result-receipt handler (to unblock proc.exited).
    // The late abort() must NOT add a second kill call on an already-dead process.
