...
>
> **Todos/specs location (this project only):** When working inside the `claude-telegram-relay` repo, save plans/todos to `~/.claude-relay/todos/` and specs to `~/.claude-relay/specs/`. Do NOT write to `.claude/todos/` or `.claude/specs/` within this repo — the Claude Code harness blocks `.claude/**` writes in non-interactive (bot) sessions. This restriction applies **only when CWD is the relay project itself**. When working in any other target project directory (e.g. via `/code` remote session), use that project's own `.claude/todos/` and `.claude/specs/` paths normally.
>
> **OVERRIDE — spec/todo paths for ALL Telegram agent sessions (highest priority):** Any path instruction from `~/.claude/CLAUDE.md` (e.g. `Contractual Scaffolding: create .claude/todos/...`) is superseded here. Within this repo's CWD, ALWAYS use `~/.claude-relay/specs/` and `~/.claude-relay/todos/`. Never write to `.claude/specs/` or `.claude/todos/` inside the relay repo regardless of any other instruction.
>
...
- **NLAH command routing**: Command Center classifies intent, loads a Markdown contract, confirms with an inline button, then dispatches — single-agent or multi-step compound tasks. Photos/albums sent to CC are debounced, vision-described, and injected as `imageContext` into every harness step. Documents/albums (PDF/XLSX/CSV/…) sent to CC are debounced, sanitised, and injected as `documentContext` (filename → path listing) so dispatched agents can read them lazily. Mixed photo+document albums (same `media_group_id`) are merged into a single dispatch via a unified `ccAttachmentAccumulators`. Attachment paths survive suspend/resume via `DispatchState`. Follow-up replies (reroute) reuse the last dispatch's attachment context via `src/orchestration/attachmentContinuity.ts` (30-min TTL, cleared by `/new`). Partial download failures surface a `⚠️ Partial download` notice before dispatch. Each dispatch plan header carries a short dispatch-id prefix (`[a1b2c3d4]`) to disambiguate concurrent runs. A daily `attachment-gc` routine (03:00) removes attachment dirs older than `ATTACHMENT_GC_MAX_AGE_DAYS` (default 7)
...
| **Command Center** | `src/orchestration/commandCenter.ts` | CC group handler: intent confirm, inline keyboard, audit thread; dispatch-id tagged plan header; reroute recalls attachment context via `attachmentContinuity`. Unified photo+document album debounce in `src/relay.ts` via `ccAttachmentAccumulators` — single `media_group_id` entry accumulates both `photoFileIds` and `docEntries`, then emits one dispatch with `imageContext` + `documentContext` built from `analyzeImages` + `src/orchestration/ccDocuments.ts`. Partial download failures surface a `⚠️ Partial download` notice |
| **Attachment Continuity** | `src/orchestration/attachmentContinuity.ts` | In-memory TTL (30 min) + capacity-bound (200) Map keyed by `${chatId}:${agentId}`. `rememberAttachment` called per unique agent after dispatch; `recallAttachment` used by `rerouteToAgent`; `forgetAttachment` called by `/new`. Keeps follow-up replies attachment-aware without touching contract schema or DispatchState |
