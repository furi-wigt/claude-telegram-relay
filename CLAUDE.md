...
### Step 3b: Customise Agent Prompt Save Paths (Optional)

Three agents save research and documentation to disk by default:
- `config/prompts/research-analyst.md` → saves to `~/Documents/ai-research/`
- `config/prompts/documentation-specialist.md` → saves to `~/Documents/ai-docs/`
- `config/prompts/security-analyst.md` → saves to `~/Documents/ai-security/`

If the user wants files saved elsewhere, edit those three files and change the `💾 Save to:` path.
...
> **Routine guide:** See `routines/user_journey.md` for a complete guide to creating, scheduling, and managing routines.
...
- **Production Routines** — the `routines/` directory has ready-to-use scheduled tasks. See `routines/user_journey.md` for a full guide to creating your own.
...
- Add new routines in `routines/` following the pattern in `routines/user_journey.md`
