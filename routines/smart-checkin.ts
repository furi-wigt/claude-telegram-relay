...
import { queryActivity } from "../src/activity/activityLog.ts";
...
async function getStaleActivityItems(): Promise<string[]> {
  try {
    ...
    const items = await queryActivity({
      chat_id: "all",
      status: ["open", "pending"],
      since: sevenDaysAgo,
      limit: 3,
    });
    return items.map((e) => `${e.event_type}: ${e.title}`);
  } catch {
    return [];
  }
}
...
async function askClaudeToDecide(): Promise<{
  ...
  const [goals, staleItems, recentMessages] = await Promise.all([
    getActiveGoals(),
...
- Stale pending activities: ${staleItems.length > 0 ? staleItems.join("; ") : "None"}
