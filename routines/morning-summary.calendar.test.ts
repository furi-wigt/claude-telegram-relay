 ...
 *
 * IMPORTANT: This file does NOT use mock.module() for claude-process.ts or
 * osx-calendar/index.ts. Instead it injects mocks through morning-summary's
 * exported _deps object. This avoids polluting bun's module cache, which
 * would break other test files (claude-process.e2e.test.ts, teamAnalyzer.test.ts,
 * osx-calendar.unit.test.ts) that need the real implementations of those modules.
 *
...
// -- Local DB mock (return empty results for activity/goals) --
...
// "../integrations/osx-calendar/index.ts" here. Instead we inject mocks
// through morning-summary's _deps export (see beforeEach below). This
// prevents those real modules from being replaced in bun's module cache.

// Restore all module mocks after this file finishes so subsequent test files
// in the same bun worker see the real modules (not this file's mocks).
afterAll(() => {
  mock.restore();
});
...
// These are ordinary mock functions — not registered via mock.module.
// They're injected into morning-summary through _deps in beforeEach.
...
beforeEach(() => {
  ...
  // Inject mocks through _deps — avoids mock.module() module-cache pollution.
  _deps.createAppleCalendarClient = mockCreateAppleCalendarClient as any;
...
// ============================================================
// getTodayCalendarEvents() — mocked calendar client via _deps
// ============================================================
