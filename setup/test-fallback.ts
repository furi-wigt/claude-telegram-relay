...
import { checkOllamaAvailable, ensureModel, callOllamaGenerate, getModel } from "../src/ollama/index.ts";

console.log("Testing fallback model configuration...\n");

const chatModel = getModel("chat-fallback");
const OLLAMA_API_URL = process.env.OLLAMA_URL ?? process.env.OLLAMA_API_URL ?? "http://localhost:11434";

console.log(`Fallback model: ${chatModel}`);
...
console.log(`Checking if ${chatModel} is available...`);
const available = await checkOllamaAvailable(chatModel);

if (!available) {
  console.log(`✗ Model ${chatModel} not found\n`);
  console.log("Attempting to pull model...");

  const pulled = await ensureModel(chatModel);
  if (!pulled) {
    console.error("✗ Failed to pull model");
    console.log(`\nTry manually: ollama pull ${chatModel}`);
    process.exit(1);
  }

  console.log("✓ Model pulled successfully\n");
} else {
  console.log(`✓ Model ${chatModel} is available\n`);
}
...
try {
  const response = await callOllamaGenerate(testPrompt, { purpose: "chat-fallback" });
